...
start_all = dpc[data.forecast_date[0]-pd.Timedelta(8,'day'):data.forecast_date[0]-pd.Timedelta(2,'day')].sum()
start_data = data.forecast_date[0]-pd.Timedelta(2,'day')
...
for t in targets:
    ...
    plt.scatter(start_data, start, color='k')
    plt.plot([start_data]+target_dates, point, '.-', markersize=10, label='SUIHTER')
    plt.fill_between([start_data]+target_dates, low, high, alpha=0.3)
    plt.legend()
    #plt.title(t)
    plt.xticks([start_data]+target_dates)
