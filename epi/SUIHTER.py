...
class SUIHTER:
    ...
    def model(self, t, y0):
        ...
        UbtoR = rho_U * Ub
        UvtoR = rho_U * Uv
        omega_I_factor = ((casesS + self.h1 * casesV1 + self.h2 * casesV2)/
                          (casesSini + self.h1 * casesV1ini + self.h2 * casesV2ini) *
                          (1 - self.xi_H * current_variant_prevalence)/
                          (1-self.xi_H*self.variant_prevalence_hosp)) if self.forecast else 1
        omega_H_factor = ((casesS + self.t1 * casesV1 + self.t2 * casesV2)/
                          (casesSini + self.t1 * casesV1ini + self.t2 * casesV2ini) * 
                          (1 - self.xi_T * current_variant_prevalence)/
                          (1-self.xi_T*self.variant_prevalence_hosp)) if self.forecast else 1
        gamma_I_factor = ((casesS + self.m1 * casesV1 + self.m2 * casesV2)/
                          (casesSini + self.m1 * casesV1ini + self.m2 * casesV2ini)) if self.forecast else 1
    ...
    def error(self, params0):
        ...
        weightsR = 0 #0.1*weight/self.data['Recovered'].max()
