...
def model_fun(q,data):
    ...
    new_pos = (res[Ns:2*Ns,:] * np.array([args[2].atTime(t).dot(args[5].transpose())[2] for t in t_data]).transpose()).flatten()
    rec_det = es.postProcessH(args[2],t_data,res[2*Ns:3*Ns,:], res[3*Ns:4*Ns,:], res[4*Ns:5*Ns,:],args[5]).flatten('F') + np.repeat(data.ydata[0][-2,0::int(args[2].dataEnd+1)],len(t_data)) 
