...
def epiMOX(testPath,params=None,ndays=None,tf=None,estim_req=None,ext_deg_in=None,scenari=None):
    ...
    #plt.plot(range(300),[params.omegaI(t) for t in range(300)], linewidth=4)
    #plt.show()
    #return
...
#        initI['New_positives'] = eData['New_positives'].iloc[-7:].mean()
        ...
        if model == 'SUIHTER':
            ...
            sol[2*Ns:3*Ns] = 0 
            sol[3*Ns:4*Ns] = 0 
            sol[4*Ns:5*Ns] = initI['Isolated'].values
            sol[5*Ns:6*Ns] = 0
            sol[6*Ns:7*Ns] = 0
            sol[7*Ns:8*Ns] = initI['Hospitalized'].values
            sol[8*Ns:9*Ns] = initI['Threatened'].values
            sol[9*Ns:10*Ns] = initI['Extinct'].values
            sol[10*Ns:11*Ns] = res[6*Ns:7*Ns,T0]
            sol[11*Ns:12*Ns] = res[7*Ns:8*Ns,T0]
            sol[12*Ns:] = res[8*Ns:,T0] 
            sol[0:Ns] = sol[0:Ns] \
                    - sol[Ns:].sum()
        res = es.solve_rk4(getattr(md, model + 'model' + model_type + '_vaccines'), [T0, Tf], sol, time_list[1]-time_list[0], l_args + (variant_perc, variant_factor, kappa1, kappa2))
        res = np.vstack([res[0:Ns],np.sum(res[Ns:4*Ns],axis=0),np.sum(res[4*Ns:7*Ns],axis=0),res[7*Ns:]])
    ### Post-process
