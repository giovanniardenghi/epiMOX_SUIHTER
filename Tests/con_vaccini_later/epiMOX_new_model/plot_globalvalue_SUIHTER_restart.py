...
for i in range(idx+1,idx+int(simdf['time'].max())):
    date=date.append(pd.Series(date[i-1]+dt,index = [i]))
simdf['data']=date.values[idx-1:]
...
ax1.plot(simdf.data,totI,'y',  label='Isolated Simulated',linewidth=2)
...
ax2.plot(simdf.data,totH,'b',  label='Hospitalized Simulated',linewidth=2)
...
ax3.plot(simdf.data,totT,'r',  label='Threatened Simulated',linewidth=2)
...
ax4.plot(simdf.data,totE,'k',  label='Extinct Simulated',linewidth=2)
...
ax5.plot(simdf.data,totDeltaU,'k',  label='New positives Simulated',linewidth=2)
...
ax6.plot(simdf.data,totRD,'g',  label='Identified Recovered Simulated',linewidth=2)
ax6.plot(simdf.data,totR,'g--',  label='Recovered Simulated',linewidth=2)
...
ax7.plot(simdf.data,totU,'k',  label='Undetected Simulated',linewidth=2)
...
vaccines = vaccines.loc[startData:simdf.data.values[-1]]
vaccines = vaccines.reindex(pd.date_range(vaccines.index[0],simdf['data'].iloc[-1])).ffill()

#ax8 = plt.subplot(248)
#ax8.plot(date,totS,'k',  label='Susceptibles',linewidth=2)
#ax8.plot(date,totV1,'k',  label='First dose simulated',linewidth=2)
#ax8.plot(date,vaccines['prima_dose']-vaccines['seconda_dose'],'k--',  label='First dose',linewidth=2)
#ax8.plot(date,totV2,'r',  label='Second dose simulated',linewidth=2)
#ax8.plot(date,vaccines['seconda_dose'],'r--',  label='Second dose',linewidth=2)
...
ax8.plot(simdf.data,NewT,'k',  label='New Threatened Simulated',linewidth=2)
ax8.xaxis.set_major_locator(mpl.dates.DayLocator(interval=15))
ax8.xaxis.set_major_formatter(mpl.dates.DateFormatter('%d/%m'))
ax8.legend()
...
plt.plot(data['data'],data.ingressi_terapia_intensiva,'r+',   label='New threatened',linewidth=2)
ax = plt.gca()
ax.plot(simdf.data,NewT,'k',  label='New Threatened Simulated',linewidth=2)
ax.xaxis.set_major_locator(mpl.dates.DayLocator(interval=15))
ax.xaxis.set_major_formatter(mpl.dates.DateFormatter('%d %b'))
ax.legend()
ax.set_title('Ingressi in terapia intensiva')
plt.show()
