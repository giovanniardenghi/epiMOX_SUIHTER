...
def epiMOX(testPath,params=None,ndays=None,tf=None,estim_req=None,ext_deg_in=None,scenari=None):
    ...
    #params.delta = si.interp1d(range(int((day_end-day_init).days)-20),[Delta_t]*(int((day_end-day_init).days)-20),fill_value="extrapolate",kind='nearest')
    #plt.plot([day_init+datetime.timedelta(days=t) for t in range(len(Delta_t))],Delta_t)
    #plt.plot([day_init+datetime.timedelta(days=t) for t in range(300)],[params.delta(t) for t in range(300)])
    #plt.show()
    #return
    ...
    #rhoI =  pd.Series((-eData['Isolated'].diff().values+eData['New_positives'].values-pd.to_numeric(ric.loc[day_init:day_end-pd.Timedelta(1,'day'),'RICOVERI'].values))/eData['Isolated'].values).rolling(center=True,window=7,min_periods=1).mean()
    #rhoH =pd.Series((eData['Recovered'].diff().values-rhoI.values*eData['Isolated'].values)/eData['Hospitalized'].values).rolling(center=True,window=7,min_periods=1).mean()
    #gammaT = pd.Series((eData['Recovered'].diff().values-eData['Isolated'].diff().values+eData['Hospitalized'].diff().values+eData['Threatened'].diff().values-2*ric.loc[day_init:day_end,'RICOVERI'].values+eData['New_positives'].values)/2/eData['Threatened'].values).rolling(center=True,window=7,min_periods=1).mean()
    #gammaT = pd.Series((-eData['Threatened'].diff().values + eData['New_threatened'].values)/eData['Threatened'].values).rolling(center=True,window=7,min_periods=1).mean()
    #gammaH = pd.Series((eData['Extinct'].diff().values-gammaT*eData['Threatened'])/eData['Hospitalized'].values).rolling(center=True,window=7,min_periods=1).mean()
    #gammaH = pd.Series((-eData['Hospitalized'].diff().values+pd.to_numeric(ric.loc[day_init:day_end-pd.Timedelta(1,'day'),'RICOVERI'].values))/eData['Hospitalized'].values - rhoH - omegaH).rolling(center=True,window=7,min_periods=1).mean()
    #gamma = eData['Extinct'].diff().rolling(center=True,window=7,min_periods=1).mean()
    #gammaT = pd.Series(((eData['Threatened'].values/8799)*0.55+0.25)/8).rolling(center=True,window=7,min_periods=1).mean()
    #gammaH = pd.Series((eData['Extinct'].diff().values-gammaT*eData['Threatened'].values)).rolling(center=True,window=7,min_periods=1).mean()

    
    #thetaT = pd.Series((eData['New_threatened'].values-eData['Threatened'].diff().values)/eData['Threatened'].values+gammaT).rolling(center=True,window=7,min_periods=1).mean()

    params.omegaI = si.interp1d(range(1,int((day_end-day_init).days-2)),omegaI,fill_value='extrapolate',kind='nearest')
    params.omegaH = si.interp1d(range(1,int((day_end-day_init).days)),omegaH,fill_value='extrapolate',kind='nearest')
    #params.omegaI = si.interp1d(range(1,int((day_end-day_init).days)),[omegaI.mean()]*(int((day_end-day_init).days)-1),fill_value='extrapolate',kind='nearest')
    #params.omegaH = si.interp1d(range(1,int((day_end-day_init).days)),[omegaH.mean()]*(int((day_end-day_init).days)-1),fill_value='extrapolate',kind='nearest')
    #params.rhoI = si.interp1d(range(int((day_end-day_init).days)),rhoI,fill_value='extrapolate',kind='nearest')
    #params.rhoH = si.interp1d(range(int((day_end-day_init).days)),rhoH,fill_value='extrapolate',kind='nearest')
    #params.gammaT = si.interp1d(range(int((day_end-day_init).days)),gammaT,fill_value='extrapolate',kind='nearest')
    #params.gammaH = si.interp1d(range(int((day_end-day_init).days)),gammaH,fill_value='extrapolate',kind='nearest')
    #params.gamma = si.interp1d(range(int((day_end-day_init).days)),gamma,fill_value='extrapolate',kind='nearest')
    #params.thetaT = si.interp1d(range(int((day_end-day_init).days)),thetaT,fill_value='extrapolate')
    
    #plt.plot(range(150),[params.delta(t) for t in range(150)], linewidth=4)
    #plt.plot(range(100),[params.omegaH(t) for t in range(100)], linewidth=4)
    #plt.show()
    #return
    
    if by_age:
        #perc = np.loadtxt('util/Percentage_by_age.txt')
        #perc_long = np.tile(perc.transpose(),(len(eData),1))
        #eData = pd.DataFrame(np.repeat(eData.values,Ns,axis=0),columns=eData.columns)
        #eData[['Isolated','Hospitalized','Threatened','Extinct','Recovered']] = eData[['Isolated','Hospitalized','Threatened','Extinct','Recovered']].values * perc_long
    ...
    #initI['Undetected'] = UD.loc[initI['data'].iloc[0][0:10]]
    initI['Undetected'] = UD.loc[dates].mean()

    #initI['Undetected'] = 12274 #Italia


    #ref = 0.4157 # Italia
    #scenari = np.array([[157,0.4395]])/np.array([1,ref])              #Scenario 1 
    #scenari = np.array([[157,0.4395],[164,0.4637]])/np.array([1,ref])              #Scenario 2 
    #scenari = np.array([[157,0.4395],[171,0.4637]])/np.array([1,ref])              #Scenario 3 
    

    #ref = 0.4321
    #scenari = np.array([[160,0.6637]])/np.array([1,ref])              #Scenario 3
    #scenari = np.array([[157,0.4395],[164,0.4732],[171,0.4778]])/np.array([1,ref])              #Scenario 4

    Recovered = (1/IFR-1)*initI['Extinct'].sum()
    initI['Recovered'] = Recovered#*initI['Recovered']/initI['Recovered'].sum()
    #initI['Recovered'] = 3370158
    #initI['Recovered'] = 2793236
    print('Undetected', initI['Undetected'])
    print('Recovered', initI['Recovered'])

    #if by_age:
    #   tmp = pd.DataFrame(np.repeat(initI.iloc[:,:-1].values, 6, axis=0), columns = initI.columns[:-1])
    #    tmp.iloc[:,3:9] *= np.repeat(perc,[2,1,1,1,1],axis=0).transpose()
    #    tmp['data'] = initI['data'].values[0]
    #    initI = tmp
    ...
    #perc_v2 = 0.35
    #vaccines.loc[pd.to_datetime('2021-06-01'):,:] = 7e5*np.array([1-perc_v2,perc_v2]) 
    #vaccines.loc[day_end:pd.to_datetime('2021-05-31')] = np.nan 
    #vaccines.loc[pd.to_datetime(DPC_ndays):pd.to_datetime('2021-06-01')] = vaccines.loc[pd.to_datetime(DPC_ndays):pd.to_datetime('2021-06-01')].interpolate() 
    ...
    print('Simulating...')
    l_args = (params,Pop,DO,map_to_prov,dv1,dv2)
    if by_age:
        model_type='_age'
    else:
        model_type=''
    ...
    if only_forecast:
        variant_perc = 0.16
        variant_factor = np.arange(15,30)/10
        kappa = np.arange(10)/10
        R_star = np.zeros((len(variant_factor),len(kappa)))
        ...
        initI['Isolated_base'] = (1 - variant_perc) * initI['Isolated']
        initI['Isolated_variant'] = variant_perc * initI['Isolated']
        ...
        if model == 'SUIHTER':
            ...
            sol[3*Ns:4*Ns] = initI['Isolated_base'].values
            sol[4*Ns:5*Ns] = initI['Isolated_variant'].values
            sol[5*Ns:6*Ns] = initI['Hospitalized'].values
            sol[6*Ns:7*Ns] = initI['Threatened'].values
            sol[7*Ns:8*Ns] = initI['Extinct'].values
            sol[8*Ns:9*Ns] = res[6*Ns:7*Ns,T0]
            sol[9*Ns:10*Ns] = res[7*Ns:8*Ns,T0]
            sol[10*Ns:] = res[8*Ns:,T0] 
            sol[0:Ns] = sol[0:Ns] \
                    - sol[Ns:2*Ns] \
                    - sol[2*Ns:3*Ns] \
                    - sol[3*Ns:4*Ns] \
                    ...
                    - sol[9*Ns:10*Ns] \
                    - sol[10*Ns:] 
        for i,vf in enumerate(variant_factor):
            for j,k in enumerate(kappa):
                res_tmp = es.solve_rk4(getattr(md, model + 'model' + model_type + '_variant'), [T0, Tf], sol, time_list[1]-time_list[0], l_args + (variant_perc, vf, k))
                res_tmp = np.vstack([res_tmp[0:Ns],np.sum(res_tmp[Ns:3*Ns],axis=0),np.sum(res_tmp[3*Ns:5*Ns],axis=0),res_tmp[5*Ns:]])
                logI = pd.Series(np.log(res_tmp[1:5].sum(axis=0)))
                gamma = 1/9
                Rt = logI.diff(periods=7) / 7 / gamma + 1

                R_star[i,j] = Rt.max()
                np.savetxt('R_star_beta_kappa',R_star)
    ### Post-process
    print(R_star)
    ...
    results[:,-3] = es.postProcessH(params, time_list, res[2*Ns:3*Ns,:], res[3*Ns:4*Ns,:], res[4*Ns:5*Ns,:], map_to_prov).flatten() + np.tile(eData[eData['time']==0]['Recovered'].values,len(time_list)).squeeze()
