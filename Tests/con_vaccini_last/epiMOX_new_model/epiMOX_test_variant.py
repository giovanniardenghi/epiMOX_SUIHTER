...
def epiMOX(testPath,params=None,ndays=None,tf=None,estim_req=None,ext_deg_in=None,scenari=None):
    ...
    eData = eData[(eData["data"]>=day_init.isoformat())]
    ...
    #ric = pd.read_csv('https://raw.githubusercontent.com/floatingpurr/covid-19_sorveglianza_integrata_italia/main/data/2021-07-05/ricoveri.csv')
    ric = ric.iloc[:-1]
    ric['DATARICOVERO1'] = pd.to_datetime(ric['DATARICOVERO1'],format='%d/%m/%Y')
    day_end = datetime.date.fromisoformat('2021-10-28')
    ...
    day_end = day_end_tmp
    eData = eData[(eData["data"]<day_end.isoformat())]
    ...
    params.omegaI = si.interp1d(range(1,len(omegaI)+1),omegaI,fill_value='extrapolate',kind='nearest')
    params.omegaH = si.interp1d(range(1,len(omegaH)+1),omegaH,fill_value='extrapolate',kind='nearest')
