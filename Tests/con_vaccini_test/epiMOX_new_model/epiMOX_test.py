...
def epiMOX(testPath,params=None,ndays=None,tf=None,estim_req=None,ext_deg_in=None,scenari=None):
    ...
    ric = pd.read_csv('https://raw.githubusercontent.com/floatingpurr/covid-19_sorveglianza_integrata_italia/main/data/2021-07-25/ricoveri.csv')
    ric = ric.iloc[:-1]
...
#        initI['New_positives'] = eData['New_positives'].iloc[-7:].mean()
        ...
        if model == 'SUIHTER':
            ...
            sol[8*Ns:9*Ns] = 0#es[7*Ns:8*Ns,T0]
            sol[9*Ns:] = 0#res[8*Ns:,T0] 
            sol[0:Ns] = sol[0:Ns] \
                    - sol[Ns:2*Ns] \
                    - sol[2*Ns:3*Ns] \
                    - sol[3*Ns:4*Ns] \
