...
from epiMOX import epiMOX,correct_isolated_model
...
if __name__ == '__main__':
    ...
    if epi_model == 'SEIRD':
        ...
        obs = np.concatenate([isolated,hospitalized,threatened, extinct_diff, recovered], axis=0).reshape((5,-1))
    nstep = int(eDataG['time'].max() / dt)
    times = np.arange(0, nstep + 1) * dt
    mcstat = solveMCMC(testPath,times, obs, Y0, l_args, nsimu=nsimu, sigma=np.array([0.1 * 3e2]*5), epi_model=epi_model, parallel=parallel, nproc=nproc, nchains=nchains)
